  ...
  fetch("https://opensheet.elk.sh/146Kh8xhTY8QB9rObYTdlmm8FDGXrs8TDjki7ph7UW2E/1")
    ...
    .then(data => {
      ...
      const parseDate = (dateStr) => {
        ...
        // DD/MM/YYYY ফরম্যাট হ্যান্ডেল করা
        return parts.length === 3 ? new Date(parts[2], parts[1]-1, parts[0]) : new Date(dateStr);
      };

      // নোটিশগুলোকে লেটেস্ট ডেট অনুযায়ী সর্ট করা
      data.sort((a,b) => parseDate(b.Date) - parseDate(a.Date));

      const LIMIT = 3;
      data.slice(0, LIMIT).forEach(row => {
        // শিটে কলামের নাম Type বা type যাই হোক তা চেক করবে
        let type = (row.Type || row.type || "").toLowerCase().trim();
        let noticeText = (row.Notice || "নোটিশ পাওয়া যায়নি").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g, "<br>");
        let dateValue = row.Date || "";

        let badge = "";
        let importantClass = "";

        // IMPORTANT ব্যাজ লজিক
        if(type === "important"){
          importantClass = "notice-important";
          badge += `<span class="notice-badge">IMPORTANT</span>`;
        }

        // NEW ব্যাজ লজিক (আজকের ডেট হলে দেখাবে)
        if(dateValue){
          const d = parseDate(dateValue);
          if(d.toDateString() === getBDTime().toDateString()){
            badge += `<span class="notice-badge notice-new">NEW</span>`;
          }
        }

        // বাংলা তারিখ ফরম্যাট
        let showDate = dateValue ? parseDate(dateValue).toLocaleDateString("bn-BD",{day:"numeric", month:"long", year:"numeric"}) : "";
        
        let shortText = noticeText.length > 120 ? noticeText.substring(0,120) + "..." : noticeText;
